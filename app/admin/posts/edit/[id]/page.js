...
export default function EditPost() {
  ...
  const handleDelete = async () => {
    if (!confirm('Destroy this article permanently?')) return;
    try {
      const res = await fetch(`/api/posts/${id}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
        toast.success('Incinerated');
        router.push('/admin/dashboard');
      }
    } catch (err) {
      toast.error('Connection error');
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 border-2 border-[#ac8aff] border-t-transparent rounded-full animate-spin"></div>
          <span className="text-zinc-500 font-mono tracking-widest text-xs uppercase">Deciphering...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
      <header className="sticky top-0 z-50 bg-[#111111]/80 backdrop-blur-xl border-b border-white/5 py-4 px-8 flex items-center justify-between">
        <div className="flex items-center gap-6">
          ...
          <h1 className="text-xl font-bold tracking-tight">Modify Infrastructure</h1>
        </div>

        <div className="flex items-center gap-4">
          <button onClick={handleDelete} className="p-2 text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all" title="Delete Permanently">
            <Trash2 size={20} />
          </button>
